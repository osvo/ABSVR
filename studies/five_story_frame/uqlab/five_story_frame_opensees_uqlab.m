function Y = five_story_frame_opensees_uqlab(X)
%FIVE_STORY_FRAME_OPENSEES_UQLAB Evaluate physical frame inputs in OpenSeesPy.

    arguments
        X (:,21) double {mustBeFinite, mustBePositive}
    end

    thisFile = mfilename('fullpath');
    uqlabDirectory = fileparts(thisFile);
    frameDirectory = fileparts(uqlabDirectory);
    studiesDirectory = fileparts(frameDirectory);
    repoRoot = fileparts(studiesDirectory);
    ...
    command = sprintf( ...
        '"%s" -m studies.five_story_frame.opensees_batch_bridge --input "%s" --output "%s"%s', ...
